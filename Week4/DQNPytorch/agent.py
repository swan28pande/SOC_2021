...
class agent():
    ...
    def get_action(self,game_board,eps):
        # Given the Board State, get invalid actions for that state
        invalid_actions = []
        for idx,action in enumerate(action_map):
            if(not game_board.check_move(action)):
                invalid_actions.append(idx)

        # Either Sample a Random Valid Action or take the action prescribed by DQN
        # Random action with probability eps and DQN with prob. 1-eps
    ...
    def generate_episode(self,eps):
        ...
        while(game_board.check_state()):
            ...
            #Keeping Track of the States, Actions and Rewards for all the transitions in the Episode
    ...
    def train(self,n_epochs=1000):
        # Generate 10 episodes in an Epoch - use those episodes to train the DQN
        """
