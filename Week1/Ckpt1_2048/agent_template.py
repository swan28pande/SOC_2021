...
def expecti_minimax(board_state, is_current, depth_rem):
    ...
    if depth_rem == 0 or not board_state.check_state():
        ...
        if is_current:
            ...
            bs1.update('L')
            left = expecti_minimax(bs1, False, depth_rem)[0] if board_state.check_move('L') \
                else -1
            bs2 = copy.deepcopy(board_state)
            bs2.update('R')
            right = expecti_minimax(bs2, False, depth_rem)[0] if board_state.check_move('R') \
                else -1
            bs3 = copy.deepcopy(board_state)
            bs3.update('U')
            up = expecti_minimax(bs3, False, depth_rem)[0] if board_state.check_move('U') \
                else -1
            bs4 = copy.deepcopy(board_state)
            bs4.update('D')
            down = expecti_minimax(bs4, False, depth_rem)[0] if board_state.check_move('D') \
                else -1
        ...
        # Returns Score as well as a 'None' Move
            ...
            bs1 = copy.deepcopy(board_state)
            bs1.update('L')
            left = expecti_minimax(bs1, True, depth_rem - 1)[0] if board_state.check_move('L') \
                else -1
            bs2 = copy.deepcopy(board_state)
            bs2.update('R')
            right = expecti_minimax(bs2, True, depth_rem - 1)[0] if board_state.check_move('R') \
                else -1
            bs3 = copy.deepcopy(board_state)
            bs3.update('U')
            up = expecti_minimax(bs3, True, depth_rem - 1)[0] if board_state.check_move('U') \
                else -1
            bs4 = copy.deepcopy(board_state)
            bs4.update('D')
            down = expecti_minimax(bs4, True, depth_rem - 1)[0] if board_state.check_move('D') \
                else -1
            ctr = 0
            tot = 0
